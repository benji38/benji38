...
function AWarn:CheckForPunishment( pl, warnings )
	...
	if self.Punishments[ warnings ].pType == "kick" then
		...
		local str = ("**%s** was kicked from the server for exceeding an active warning threshold.\nActive Warning Count: **%u**"):format(pl:GetName(), warnings)
		AWarn:DiscordPunishment( str )
	...
	elseif self.Punishments[ warnings ].pType == "ban" then		
		...
		local str = ("**%s** was banned from the server for exceeding an active warning threshold.\nActive Warning Count: **%u**"):format(pl:GetName(), warnings)
		AWarn:DiscordPunishment( str )
